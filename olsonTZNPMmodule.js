	...
	for(var i = 0; i < wishedTimezonesArray.length; i++)
	{
		...
		for(var j = 0; j < timezones.length; j++)
		{
			...
			if(wishedTimezonesArray[i] === timezones[j].timezone) {
				...
				// Split the current timezone from 'String' into an array of 'Strings' named 'treatmentArray'...
				var treatmentArray = wishedTimezonesArray[i].split("/");
				
				// In the case where the 'treatmentArray' array contains only one element...
				if(treatmentArray.length === 0) {

					// Add the current timezone to the 'geographicRegion' String variable...
					geographicRegion = wishedTimezonesArray[i];
...
function getGeographicRegionFromMultipleTimezones(wishedTimezonesArray) {
	...
	// Definition of the 'treatmentArray' variable which will be an array to extract geographical region...
	var treatmentArray = null;
	...
	for(var i = 0; i < wishedTimezonesArray.length; i++)
	{
		...
		for(var j = 0; j < timezones.length; j++)
		{
			...
			if(wishedTimezonesArray[i] === timezones[j].timezone) {
				...
				// Split the current timezone into 'treatmentArray'...
				treatmentArray = wishedTimezone.split("/");
			
				// In the case where the 'treatmentArray' array contains only one element...
				if(treatmentArray.length === 0) {

					// Add the timezone to the 'wishedTimezonesGeographicRegionsJSON' JSON variable...
					wishedTimezonesGeographicRegionsJSON[timezones[j].timezone] = wishedTimezone;
